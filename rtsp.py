...
class SensorFactory(GstRtspServer.RTSPMediaFactory):
    def __init__(self, **properties):
        ...
        self.launch_string = 'appsrc name=mysrc is-live=true block=true format=GST_FORMAT_TIME ' \
                             'caps=video/x-raw,format=BGR,width={},height={},framerate={}/1 ' \
                             '! videoconvert ! video/x-raw,format=I420 ' \
                             '! x264enc speed-preset=ultrafast tune=zerolatency ' \
                             '! rtph264pay config-interval=1 name=pay0 pt=96' \
            .format(opt.image_width, opt.image_height, self.fps)
        self.cap = cv2.VideoCapture('http://50.248.1.46:8000/mjpg/video.mjpg')
        # self.cap.set(cv2.CAP_V4L2, 1)
        print()
    ...
    def on_need_data(self, src, length):
        if self.cap.isOpened():
            ...
            if ret:
                ...
                print('{} - pushed buffer, frame {}, duration {} ns, durations {} s'.format(datetime.now(),
                                                                                            self.number_frames,
                                                                                            self.duration,
...
parser.add_argument("--device_id", default=0, help="device id for the video device or video file location")
